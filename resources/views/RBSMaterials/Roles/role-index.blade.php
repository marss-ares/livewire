<div>
    <header class="relative mb-6 flex flex-col gap-4">
        <div class="flex items-center justify-between w-full">
            ...
            <div class="flex-none">
                <flux:input size="sm" icon="magnifying-glass" placeholder="Search roles..." wire:model.live="search"
                    class="w-64" />
            </div>
        </div>
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700/50">
                @forelse ($roles as $role)
                    <tr wire:key="role-{{ $role->id }}"
                        ...
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">
                                {{ $role->permissions_count }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300">
                                {{ $role->users_count }}
                            </span>
                        </td>
